...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        """
        # Add set() to allocate space for new layer
        self.a_levels.append(set())
        for a in self.all_actions:
            PG_note_a = PgNode_a(a)
            if PG_note_a.prenodes.issubset(self.s_levels[level]):
                self.a_levels[level].add(PG_note_a)
                for prenode in PG_note_a.prenodes:
                    for state_node in self.s_levels[level]:
                        if(state_node.__eq__(prenode)):
                            state_node.children.add(PG_note_a)
                            PG_note_a.parents.add(state_node)
        pass
    ...
    def add_literal_level(self, level):
        ...
        for PG_node in self.a_levels[level-1]:
            PG_node_set = PG_node.effnodes
            for output_state in PG_node_set:      
                self.s_levels[level].add(output_state)          
                PG_node.children.add(output_state)
                output_state.parents.add(PG_node)
        pass
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for eff in node_a1.effnodes:
            for eff2 in node_a2.effnodes:
                if ((eff.symbol==eff2.symbol) and (eff.is_pos == (not eff2.is_pos))): 
                    return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for eff in node_a1.effnodes:
            for eff2 in node_a2.prenodes:
                if ((eff.symbol==eff2.symbol) and (eff.is_pos == (not eff2.is_pos))): return True
        for eff in node_a2.effnodes:
            for eff2 in node_a1.prenodes:
                if ((eff.symbol==eff2.symbol) and (eff.is_pos == (not eff2.is_pos))): return True
        return False
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        parent1 = node_a1.parents
        parent2 = node_a2.parents
        for p1 in parent1:
            for p2 in parent2:
                if p1.is_mutex(p2):
                    return True
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        parent1 = node_s1.parents
        parent2 = node_s2.parents
        for p1 in parent1:
            for p2 in parent2:
    ...
    def h_levelsum(self) -> int:
        ...
        max_depth = len(self.s_levels)
        for depth in range(max_depth):
            for state in self.s_levels[depth]:
                for idx, goal_state in enumerate(goal_list):
                    if (state.symbol in self.problem.goal) and (state.is_pos==True):
                        level_sum = level_sum + depth
                        goal_list.pop(idx)
        return level_sum